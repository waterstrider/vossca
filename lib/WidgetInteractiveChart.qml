...
//    }
    ...
    AreaSeries {
        color: "#44D3AB21"; borderColor: "#FFD3AB21"
        borderWidth: 1
        axisX: _xAxis; axisY: _yAxis
        visible: false
...
//        }
    ...
    AreaSeries {
        color: "#8821D3AB"; borderColor: "#FF21D3AB"
        borderWidth: 1
        axisX: _xAxis; axisY: _yAxis
        visible: false
//        lowerSeries: LineSeries {
...
//        }
    ...
    AreaSeries {
        color: "#CCAB21D3"; borderColor: "#FFAB21D3"
        borderWidth: 1
        axisX: _xAxis; axisY: _yAxis
        visible: false
//        lowerSeries: LineSeries {
...
//        }
    ...
    LineSeries {
        ...
        axisX: _xAxis; axisY: _yAxis
        width: 2
        XYPoint { x: cursorPosition; y:_yAxis.min }
        XYPoint { x: cursorPosition; y:_yAxis.max }
    }
    ...
    function addSeries(name, title, borderColor, color) {
        if(_seriesCount >= chartView.count-1) return false;

        var newSeries = chartView.series(_seriesCount);
        _seriesCount++;
        newSeries.name = title;
        if(typeof color !== 'undefined') newSeries.color = color;
        if(typeof borderColor !== 'undefined') newSeries.borderColor = borderColor;
