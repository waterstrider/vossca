...
Database {
    ...
    SqlModel {
        ...
        query: "SELECT entryNo,id,name,completed,stageHistory,trx,lastUpdateTime FROM "
               + ccsTcDatabase.databaseName + ".tcidx ORDER BY lastUpdateTime ASC"
    ...
    function seek(time) {
        ...
        while (currentIndex < rowCount) {
            ...
            var updateTime = Number(strUpdateTime.substr(
                                        0, strUpdateTime.length - 6))
            row["lastUpdateTime"] = (new Date(updateTime)).toISOString(
                        ).substr(11, 5)
