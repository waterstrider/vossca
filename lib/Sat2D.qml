...
Rectangle {
    ...
    function startup() {
        ...
        var tle_line_1 = '1 33396U 08049A   15202.00000000  .00000000  00000-0 +23387-9 2    13'
        var tle_line_2 = '2 33396  98.7684 268.1312 0001309 101.4351 221.0870 14.20004741    13'
        var sat = Satellite.satellite

        var satrec = sat.twoline2satrec(tle_line_1, tle_line_2)

        var position_and_velocity = sat.propagate(satrec, now.getUTCFullYear(),
                                                  now.getUTCMonth() + 1,
                                                  now.getUTCDate(),
                                                  now.getUTCHours(),
                                                  now.getUTCMinutes(),
        ...
        var gmst = sat.gstime_from_date(now.getUTCFullYear(),
                                        now.getUTCMonth() + 1,
    ...
    function drawLine(ctx, ww, hh, xx, yy) {
        ...
        //Future
        for (i = 1; i < 156; i++) {
            position_and_velocity = sat.propagate(satrec, now.getUTCFullYear(),
                                                  now.getUTCMonth() + 1,
                                                  now.getUTCDate(),
                                                  now.getUTCHours(),
                                                  now.getUTCMinutes() + i,
            ...
            gmst = sat.gstime_from_date(now.getUTCFullYear(),
                                        now.getUTCMonth() + 1,
                                        now.getUTCDate(), now.getUTCHours(),
                                        now.getUTCMinutes() + i,
                                        now.getUTCSeconds())
    ...
    Image {
        ...
        Canvas {
            ...
            Timer {
                ...
                running: true
                repeat: true
                onTriggered: sat_line.requestPaint()
            }
        }
        Image {
            id: station
