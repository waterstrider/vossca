...
Database {
    ...
    SqlModel {
        id: sqlTmData
        async: true
        query: "SELECT name, rawValue, engValue, generationTime, rawValidity, engValidity, receivedTime  FROM " + ccsTmDatabase.databaseName
               + ".tm_param_af WHERE (name IN (" + context.paramListString
               + ")) AND (receivedTime>0) AND (generationTime>0) ORDER BY receivedTime"
    ...
    function getTmData(time) {
        ...
        var snapshotIndex = Math.floor(
                    (time - ccsTmDatabase.startTime) / snapshotInterval)
    ...
    function skipNextTmPacket() {
        ...
        if (ret)
            ccsTmDatabase.currentIndex++
        return ret
    }
    function convertTime(row) {
        var strGenTime = "" + row["generationTime"]
        var strRecTime = "" + row["receivedTime"]
        row["generationTime"] = Number(strGenTime.substr(0,
                                                         strGenTime.length - 6))
        row["receivedTime"] = Number(strRecTime.substr(0,
                                                       strRecTime.length - 6))
    }
}
