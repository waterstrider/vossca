...
Item {
        ...
        if (sessionName !== null && typeof sessionName !== "undefined"
            ...
            if (ccsTmDatabase) {
                startTime = Qt.binding(function () {
                    return ccsTmDatabase.startTime
                })
                endTime = Qt.binding(function () {
                    return ccsTmDatabase.endTime
                })
                status = Qt.binding(function () {
                    return ccsTmDatabase.status
                })
                progress = Qt.binding(function () {
                    return ccsTmDatabase.progress
                })
    ...
    function initializePlayer() {
        if (ccsTmDatabase.status == "loaded"
            ...
            //countdown.reload(ccsTmDatabase.startTime, "THEOS" , "Sriracha");
    ...
    function initializeParamDict() {
        for (var i = 0; i < context.paramList.length; i++) {
            ...
            if (!(paramName in paramDict)) {
                paramDict[paramName] = comTmParam.createObject(container, {
                                                                   name: paramName
                                                               })
            } else {
    ...
    Timer {
        ...
        onTriggered: {
            if (playing) {
                currentTime = Math.min(currentTime + (timer.interval * speed),
                                       container.endTime)
                ...
                while (true) {
                    ...
                    if (nextTmPacket["receivedTime"] > currentTime)
                        break
